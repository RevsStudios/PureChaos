...
public class MeganiumFishingBobberEntity extends FishingBobberEntity {
	protected boolean openLava = true;
	protected int outOfLavaTime = 0;

	public MeganiumFishingBobberEntity(PlayerEntity pPlayer, World pLevel, int pLuck, int pLureSpeed) {
		super(pPlayer, pLevel, pLuck, pLureSpeed);
	}

	public MeganiumFishingBobberEntity(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
	...
	protected boolean shouldStopFishing(PlayerEntity ownerPlayer) {
		...
		boolean isMainHandEquipped = mainHandStack.getItem().equals(PCItems.MEGANIUM_FISHING_ROD.get());
		boolean isOffhandEquipped = offhandStack.getItem().equals(PCItems.MEGANIUM_FISHING_ROD.get());
